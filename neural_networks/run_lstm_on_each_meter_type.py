...
class Experiment:
    ...
    def run(self) -> None:
        ...
        for meter in candidate_meters:
            ...
            character_tensors = np.array(
                [d["characters"] for d in candidate_meter_lines]
            )
            ...
            for fold, (train_idx, test_idx) in enumerate(kf.split(syllable_tensors)):
                ...
                syllable_train, syllable_test = (
                    syllable_tensors[train_idx],
                    syllable_tensors[test_idx],
                )
                word_train, word_test = word_tensors[train_idx], word_tensors[test_idx]
                label_train, label_test = (
                    label_tensors[train_idx],
                    label_tensors[test_idx],
                )
                character_train, character_test = (
                    character_tensors[train_idx],
                    character_tensors[test_idx],
                )
                ...
                y_pred, y_true = generate_y_pred_true(
                    model, [syllable_test, word_test, character_test], label_test
                )
