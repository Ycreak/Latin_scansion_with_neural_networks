...
class TFLSTM:
    ...
    def create_model_with_word_tensors(self, dataset: LSTMDataset) -> tf.keras.models.Model:
        ...
        character_encoding_dim = len(dataset.list_with_poetry_objects[0]['characters'][0])
        character_input_layer = tf.keras.layers.Input(shape=(character_input_length, character_encoding_dim), name='character_input')
        ...
        concatenated = tf.keras.layers.concatenate([syllable_embedding, word_embedding, character_dense])
        ...
        # Create the model
        model = tf.keras.models.Model(inputs=[syllable_input_layer, word_input_layer, character_input_layer], outputs=output_layer)
